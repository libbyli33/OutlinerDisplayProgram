...
//read file
var file, reader, lines; //declare global variables
var textColor;
    ...
    //alert(lines.length);
        ...
        while (count < lines.length && lines[count].includes("/t")) {
            ...
            while (count < lines.length && lines[count].includes("/p")) {
                ...
                while (count < lines.length && lines[count].includes("/d")) {
                    ...
                    if (isFirstDetail) {
                        ...
                        if (isLastDetail) {
                            $("#newDetail").attr("class", "details lastDetail");
                        }
                    }
                    //console.log(count);
                ...
                if (isFirstPoint) {
                    ...
                    if (isLastPoint) {
                        $("#newPoint").attr("class", "subpoints lastPoint");
                    }
                }
            ...
            if (isFirstTopic) {
                ...
                if (isLastTopic) {
                    $("#newTopic").attr("class", "maintopics lastTopic");
                    $("#newTopic").removeAttr("id");
                    $("#newSubList").removeAttr("id");
                }
                else {
	...
	$(document).keydown(function(keyPressed) {
        ...
        //press left
            ...
            //if this is the case to move from the 3rd-layer to the 2nd-layer
                ...
                $(".midPanel").show();
                $(".rightPanel").show();
                //console.log($(".lastPointGet").attr("data-layer"));
...
//text-to-speech when pressing the space key
function play() {
	$(document).keydown(function(keyPressed) {
...
function changeFonts(lines){
    var fontInfo = lines[0];
    var idx;
    if (!(fontInfo == "")){
...
function changePanelProportion(lines, layerNum) {
    var proportionInfo = lines[2];
    //console.log(proportionInfo);
